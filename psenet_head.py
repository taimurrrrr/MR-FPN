...
class PSENet_Head(nn.Module):
    def __init__(self,
        ...
        self.conv1 = nn.Conv2d(in_channels, hidden_dim, kernel_size=3, stride=1, padding=1)  # 1024->256
        self.bn1 = nn.BatchNorm2d(hidden_dim)
        self.relu1 = nn.ReLU(inplace=True)

        self.conv2 = nn.Conv2d(hidden_dim, num_classes, kernel_size=1, stride=1, padding=0)  # 256->7
    ...
    def get_results(self, out, img_meta, cfg, image):
        outputs = dict()
        ...
        #print('原图：%s 现图：%s 比例：%s'%(org_img_size,img_size,scale))
       ...
       # image_1 = image.squeeze(0).permute(1,2,0)
        # image_1=(image_1.data.cpu().numpy() * 255).astype(np.uint8)
        #cv2.drawContours(image_1, bboxes_1, -1, (0, 0, 255), 3)
        #cv2.imwrite("vis.jpg", image_1)
